...
class Solution {
  public:
    Node* constructLinkedMatrix(vector<vector<int>>& mat) {
        // code Here
        vector<vector<Node* >>matrix(mat.size() , vector<Node*>(mat.size(),NULL));
        
        for(int i = 0; i<mat.size();i++){
            for(int j = 0;j<mat.size();j++){
                matrix[i][j] = new Node(mat[i][j]);
            }
        }
        
        for(int i = 0;i<mat.size();i++){
            for(int j = 0;j<mat.size();j++){
                if(i+1 < mat.size()) matrix[i][j] ->down = matrix[i+1][j];
                if(j+1 <mat.size()) matrix[i][j] ->right= matrix[i][j+1];
            }
        }
        Node* ans = matrix[0][0];
